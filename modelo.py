...
def preparar_productos(
    ...
    for indice, fila in enumerate(filas_validas(productos), start=1):
        ...
        markup = (precio_neto / costo_neto - 1.0) if costo_neto else 0.0
        preparados.append(
            {
...
def ejecutar_modelo(
    ...
    *,
    ...
    meses_buenos = {entero(x) for x in (parametros.get("meses_buenos") or [])}
    meses_malos = {entero(x) for x in (parametros.get("meses_malos") or [])}
    ...
    cobertura_inventario = max(0.0, numero(parametros.get("cobertura_inventario_meses"), 1.0))
    inventario_inicial_bruto = max(0.0, numero(parametros.get("inventario_inicial_con_iva")))
    capital_operativo = max(0.0, numero(parametros.get("capital_operativo_inicial")))
    recuperacion_inventario = min(
        1.0, max(0.0, numero(parametros.get("recuperacion_inventario_pct"), 100.0) / 100.0)
    )
    recuperacion_capital = min(
        1.0, max(0.0, numero(parametros.get("recuperacion_capital_pct"), 100.0) / 100.0)
    )
    ...
    if coincidencias:
        nombres = ", ".join(MESES[m - 1] for m in coincidencias if 1 <= m <= 12)
        advertencias.append(
    ...
    credito_iva_inicial = iva_incluido(inversion_inicial_bruta) + iva_incluido(inventario_inicial_bruto)
    desembolso_inicial = (
        inversion_inicial_bruta + inventario_inicial_bruto + capital_operativo
    )
    ...
    for indice in range(horizonte):
        ...
        if numero_mes in meses_buenos and numero_mes not in meses_malos:
            ...
            tipo_mes = "Normal"
        cantidades_mes = [
            float(p["Cantidad mes 1"]) * ((1.0 + crecimiento) ** indice) * factor_estacional
    ...
    for cantidades_mes in cantidades_por_mes:
        ventas_brutas_lista.append(
            sum(q * float(p["Precio con IVA"]) for q, p in zip(cantidades_mes, productos_preparados))
        )
        costo_vendido_bruto_lista.append(
            sum(
                q * float(p["Costo unitario con IVA"])
                for q, p in zip(cantidades_mes, productos_preparados)
            )
        )
    ...
    for indice in range(horizonte):
        ...
        if indice + 1 < horizonte:
            costo_siguiente = costo_vendido_bruto_lista[indice + 1]
        else:
            costo_siguiente = costo_vendido_bruto
        inventario_objetivo = costo_siguiente * cobertura_inventario
        compras_brutas = max(0.0, costo_vendido_bruto + inventario_objetivo - inventario_bruto)
        ...
        ire = max(0.0, resultado_operativo) * tasa_ire
        ...
        if mes == horizonte:
            recuperaciones = (
                sin_iva(inventario_final_bruto) * recuperacion_inventario
                + capital_operativo * recuperacion_capital
                + valor_residual
            )
    ...
    if any(f < 0 for f in flujos) and any(f > 0 for f in flujos):
        try:
    ...
    cantidad_base = sum(float(p["Cantidad mes 1"]) for p in productos_preparados)
    contribucion_base = sum(
        float(p["Cantidad mes 1"])
        * (float(p["Precio sin IVA"]) - float(p["Costo unitario sin IVA"]))
        for p in productos_preparados
    )
    ...
    margen_contribucion = (
        contribucion_base
        / sum(float(p["Cantidad mes 1"]) * float(p["Precio sin IVA"]) for p in productos_preparados)
        if productos_preparados
        and sum(float(p["Cantidad mes 1"]) * float(p["Precio sin IVA"]) for p in productos_preparados)
        else 0.0
    )
    ...
    roi = resultado_neto_total / inversion_base if inversion_base else None
    ...
    if any(fila["Flujo acumulado"] < 0 for fila in filas_flujo):
        ...
        if abs(minimo) > desembolso_inicial + capital_operativo:
            advertencias.append(
    ...
    indicadores = {
        ...
        "flujo_minimo_acumulado": min([flujos[0]] + [f["Flujo acumulado"] for f in filas_flujo]),
    }
...
def tabla_sensibilidad(
    ...
    base_productos, _ = preparar_productos(productos)
    precios_base = [float(p["Precio sin IVA"]) for p in base_productos]
